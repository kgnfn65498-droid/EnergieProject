...
def test_release_identity_and_ha_default():
    assert (ROOT/'VERSIE.txt').read_text().strip()=='32.4.7'
    assert 'version: "32.4.7"' in (ROOT/'slimmemeterportal_import/config.yaml').read_text()
    assert 'github_publication_enabled: false' in (ROOT/'slimmemeterportal_import/config.yaml').read_text()
    assert 'APP_VERSION = "32.4.7"' in (ROOT/'slimmemeterportal_import/rootfs/app/main.py').read_text()
