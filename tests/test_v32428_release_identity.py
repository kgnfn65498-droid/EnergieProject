...
def test_32428_release_identity_is_synchronized():
    ...
    assert 'APP_VERSION = "32.4.46"' in (APP / "main.py").read_text(encoding="utf-8")
    assert 'TARGET_RELEASE_VERSION = "32.4.46"' in (APP / "mode_entrypoint.py").read_text(encoding="utf-8")
    assert contract.CURRENT_RELEASE == "32.4.46"
    assert contract.CURRENT_PM_VERSION == "2.0.0-rc33"
    assert (APP / "projectmanager_v2/VERSION.txt").read_text(encoding="utf-8").strip() == "2.0.0-rc33"
...
def test_32428_changelog_describes_pm_observation_dependency_fix():
    ...
    assert root_log.startswith('## 32.4.46')
