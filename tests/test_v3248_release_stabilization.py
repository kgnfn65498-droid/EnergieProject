...
def test_v3248_release_identity_is_consistent():
    ...
    assert 'version: "32.4.8"' in CONFIG.read_text(encoding="utf-8")
    assert 'APP_VERSION = "32.4.8"' in (APP / "main.py").read_text(encoding="utf-8")
    assert 'TARGET_RELEASE_VERSION = "32.4.8"' in (APP / "mode_entrypoint.py").read_text(encoding="utf-8")
    assert (APP / "projectmanager_v2" / "VERSION.txt").read_text(encoding="utf-8").strip() == "2.0.0-rc5"
...
''' % str(APP)
    ...
    assert result.stdout.strip() == "32.4.8"
