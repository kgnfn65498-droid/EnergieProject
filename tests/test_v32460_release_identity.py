...
def test_replacement_60_release_identity_is_coherent():
    ...
    assert (ROOT / "CHANGELOG.md").read_text(encoding="utf-8").splitlines()[0] == f"## {TARGET} — combined autonomous publisher closure"
    assert (ROOT / "slimmemeterportal_import/CHANGELOG.md").read_text(encoding="utf-8").splitlines()[2] == f"## {TARGET} — combined autonomous publisher closure"
