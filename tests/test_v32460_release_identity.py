...
def test_62_release_identity_is_coherent():
    ...
    assert (ROOT / "CHANGELOG.md").read_text(encoding="utf-8").splitlines()[0] == f"## {TARGET} — release-chain closure and predecessor-proof hardening"
    assert (ROOT / "slimmemeterportal_import/CHANGELOG.md").read_text(encoding="utf-8").splitlines()[2] == f"## {TARGET}"
