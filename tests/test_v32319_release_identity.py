...
def test_current_release_identity_is_v32319():
    ...
    assert 'TARGET_RELEASE_VERSION = "32.3.30"' in (ROOT / "slimmemeterportal_import/rootfs/app/mode_entrypoint.py").read_text(encoding="utf-8")
    assert 'APP_VERSION = "32.3.30"' in (ROOT / "slimmemeterportal_import/rootfs/app/main.py").read_text(encoding="utf-8")
...
def test_v32319_changelog_documents_crash_recovery_hardening_and_auto_hold():
    ...
    assert root_change.startswith("## 32.3.30")
