...
def minimal_root(root: Path) -> tuple[Path, Path]:
    ...
    (root / "App/VERSIE.txt").write_text("32.4.31\n", encoding="utf-8")
    (root / "Infra").mkdir()
    for rel in (
...
def test_unreadable_candidate_becomes_review_without_aborting_plan(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    plan = clearup.build_clearup_plan(tmp_path, current_version="32.4.31", keep_rollbacks=3)
...
def test_32431_release_identity_and_cross_chat_platform_rule():
    ...
    assert 'APP_VERSION = "32.4.31"' in main
    assert 'TARGET_RELEASE_VERSION = "32.4.31"' in mode
