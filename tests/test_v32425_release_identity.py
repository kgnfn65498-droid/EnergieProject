...
def test_32425_release_identity_and_core_boundary():
    ...
    assert 'APP_VERSION = "32.4.43"' in main
    assert 'PRODUCTION_CORE_REVISION = "9.4-core3"' in main
    assert 'TARGET_RELEASE_VERSION = "32.4.43"' in mode
    assert (APP / "projectmanager_v2/VERSION.txt").read_text(encoding="utf-8").strip() == "2.0.0-rc30"


def test_32425_current_release_contract_and_changelog():
    import release_test_contract as contract
    assert contract.CURRENT_RELEASE == "32.4.43"
    assert contract.CURRENT_PM_VERSION == "2.0.0-rc30"
    ...
    assert root_log.startswith("## 32.4.43")
