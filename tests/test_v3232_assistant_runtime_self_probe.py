...
def passing_calls():
    return {
        "health": {
            "http_status": 200,
            "json": {"status": "ready", "version": "32.3.3", "read_only": True},
        },
        "august_gas": {
            "http_status": 200,
            "json": {
...
def test_acceptance_evaluation_requires_all_runtime_evidence():
    ...
    result = m.evaluate_assistant_runtime_acceptance(passing_calls(), expected_version="32.3.3")
...
def test_acceptance_fails_if_august_is_not_quarter_hour_partial():
    ...
    result = m.evaluate_assistant_runtime_acceptance(calls, expected_version="32.3.3")
...
def test_release_identity_and_changelog_are_v3232():
    ...
    assert changelog.startswith("## v32.3.3 — Assistant runtime mount-timing hotfix")
