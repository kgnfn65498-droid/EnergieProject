...
def test_version_matches():
    ...
    assert cfg_version == app_version == "32.2.1"
...
def test_version_7_0_1_matches():
    ...
    main = (ADDON / "rootfs/app/main.py").read_text(encoding="utf-8")
    assert 'version: "32.2.1"' in config
    assert 'APP_VERSION = "32.2.1"' in main
...
def test_v2060_changelog_latest_only():
    c=(ROOT/"slimmemeterportal_import/CHANGELOG.md").read_text(encoding="utf-8")
    assert "## 32.2.1" in c and c.count("\n## ") == 1
...
def test_v2440_version_and_changelog_consistency():
    ...
    assert 'version: "32.2.1"' in config
    assert 'APP_VERSION = "32.2.1"' in main
    assert "## 32.2.1" in changelog and changelog.count("\n## ") == 1
...
def test_v32023_ha_app_owns_processed_retention():
    ...
    for required in [
        ...
        'HA-app processed-retentie v32.2.1: OK',
        'HA-app processed-retentie v32.2.1: FOUT',
    ]:
        assert required in source
