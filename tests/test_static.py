...
def test_version_matches():
    ...
    assert cfg_version == app_version == "10.5.7"
...
def test_production_release_has_no_experimental_stage():
    config = (ADDON / "config.yaml").read_text(encoding="utf-8")
    assert 'version: "10.5.7"' in config
...
def test_version_7_0_1_matches():
    ...
    main = (ADDON / "rootfs/app/main.py").read_text(encoding="utf-8")
    assert 'version: "10.5.7"' in config
    assert 'APP_VERSION = "10.5.7"' in main
...
def test_v815_production_certificate_management_present():
    source = (ADDON / "rootfs/app/main.py").read_text(encoding="utf-8")
    assert 'APP_VERSION = "10.5.7"' in source
...
def test_v1057_analysis_context_contains_existing_epex_price_context():
    ...
    assert '_price_file_metrics(epex_electricity_path)' in source
    assert '_price_file_metrics(epex_gas_path)' in source
    assert "geen leveranciersopslag, belasting of totale energiekosten" in source.lower()
