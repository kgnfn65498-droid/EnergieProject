...
def test_version_matches():
    ...
    assert cfg_version == app_version == "32.2.2"
...
def test_v2060_changelog_latest_only():
    c=(ROOT/"slimmemeterportal_import/CHANGELOG.md").read_text(encoding="utf-8")
    assert "## 32.2.2" in c and c.count("\n## ") == 1
...
def test_v2440_version_and_changelog_consistency():
    ...
    assert 'version: "32.2.2"' in config
    assert 'APP_VERSION = "32.2.2"' in main
    assert "## 32.2.2" in changelog and changelog.count("\n## ") == 1
...
def test_v32023_ha_app_owns_processed_retention():
    ...
    for required in [
        ...
        'HA-app processed-retentie v32.2.2: OK',
        'HA-app processed-retentie v32.2.2: FOUT',
    ]:
        assert required in source
