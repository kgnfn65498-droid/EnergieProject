...
def test_version_matches():
    ...
    assert cfg_version == app_version == "32.4.10"
...
def test_version_7_0_1_matches():
    ...
    main = (ADDON / "rootfs/app/main.py").read_text(encoding="utf-8")
    assert 'version: "32.4.10"' in config
    assert 'APP_VERSION = "32.4.10"' in main
...
def test_v2060_changelog_latest_only():
    c=(ROOT/"slimmemeterportal_import/CHANGELOG.md").read_text(encoding="utf-8")
    assert "## 32.4.10" in c and c.count("\n## ") == 1
...
def test_v2440_version_and_changelog_consistency():
    ...
    assert 'version: "32.4.10"' in config
    assert 'APP_VERSION = "32.4.10"' in main
    assert "## 32.4.10" in changelog and changelog.count("\n## ") == 1
