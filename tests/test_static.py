...
def test_version_matches():
    ...
    assert cfg_version == app_version == "32.0.30"
...
def test_v2400_version_consistency():
    assert (ROOT/"VERSIE.txt").read_text(encoding="utf-8").strip() == "32.0.30"
    config=(ROOT/"slimmemeterportal_import/config.yaml").read_text(encoding="utf-8")
    main=(ROOT/"slimmemeterportal_import/rootfs/app/main.py").read_text(encoding="utf-8")
    assert 'version: "32.0.30"' in config
    assert 'APP_VERSION = "32.0.30"' in main
...
def test_v32023_ha_app_owns_processed_retention():
    ...
    for required in [
        ...
        'HA-app processed-retentie v32.0.30: OK',
        'HA-app processed-retentie v32.0.30: FOUT',
    ]:
        assert required in source
