...
def test_version_matches():
    ...
    assert cfg_version == app_version == "23.1.0"
...
def test_production_release_has_no_experimental_stage():
    config = (ADDON / "config.yaml").read_text(encoding="utf-8")
    assert 'version: "23.1.0"' in config
...
def test_version_7_0_1_matches():
    ...
    main = (ADDON / "rootfs/app/main.py").read_text(encoding="utf-8")
    assert 'version: "23.1.0"' in config
    assert 'APP_VERSION = "23.1.0"' in main
...
def test_v815_production_certificate_management_present():
    source = (ADDON / "rootfs/app/main.py").read_text(encoding="utf-8")
    assert 'APP_VERSION = "23.1.0"' in source
...
def test_v1060_financial_projection_engine_is_production_active():
    ...
    assert '"engine_version": "23.1.0"' in source
...
def test_v1530_completion_gate_closes_v15_guarded():
    ...
    assert '"next_major_release": "23.1.0"' in main
...
def test_v2060_changelog_latest_only():
    c=(ROOT/"slimmemeterportal_import/CHANGELOG.md").read_text(encoding="utf-8")
    assert "## 23.1.0" in c and c.count("\n## ") == 1
...
def test_v2160_completion_gate_present():
    main=(ROOT/"slimmemeterportal_import/rootfs/app/main.py").read_text(encoding="utf-8")
    for x in ['"v21_completion_gate"','"v21_complete_external_data_gates_remain"','"v21_complete_guarded_financial_action_chain_ready_for_v22"','"next_major_release": "23.1.0"','"financial_action_selection": "ready_guarded"','"publish_only_complete_positive_financial_action": True','"manual_override_allowed": False']:
        assert x in main
