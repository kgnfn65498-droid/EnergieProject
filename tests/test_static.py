...
def test_version_matches():
    ...
    assert cfg_version == app_version == "32.3.23"
...
def test_v815_production_certificate_management_present():
    source = (ADDON / "rootfs/app/main.py").read_text(encoding="utf-8")
    assert 'APP_VERSION = "32.3.23"' in source
...
def test_v2060_changelog_latest_only():
    c=(ROOT/"slimmemeterportal_import/CHANGELOG.md").read_text(encoding="utf-8")
    assert "## 32.3.23" in c and c.count("\n## ") == 1
...
def test_v2440_version_and_changelog_consistency():
    ...
    assert 'version: "32.3.23"' in config
    assert 'APP_VERSION = "32.3.23"' in main
    assert "## 32.3.23" in changelog and changelog.count("\n## ") == 1
