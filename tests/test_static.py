...
def test_version_matches():
    ...
    assert cfg_version == app_version == CURRENT_RELEASE
...
def test_top_level_launcher_present():
    ...
    assert "python3 -u /app/mode_entrypoint.py" in text
...
def test_full_workflow_refreshes_existing_transfer():
    ...
    assert "replace_existing=True" in workflow
    assert "send_notification=False" in workflow
...
def test_v660_publishes_exact_month_output():
    ...
    assert 'NAS_DATA_ROOT / "02_Output" / "Rapportages" / month_key' in source
...
def test_v670_audit_runs_after_completed_report():
    source = (ADDON / "rootfs/app/main.py").read_text(encoding="utf-8")
    assert 'handoff.get("pre_report_validation")' in source
    assert 'or handoff.get("central_validation")' in source
    assert 'result["audit"] = audit_completed_month_workflow(' in source
    assert 'central_validation=audit_validation' in source
...
def test_v680_retention_option():
    ...
    assert 'raw.get("report_service_retention_months", 13)' in source
...
def test_v720_full_workflow_suppresses_transfer_notification():
    ...
    assert "send_notification=False" in source
    assert "report_validation=pre_report_validation" in source
...
def test_v32034_historical_report_uses_metric_fallback_before_skip():
    source = (ADDON / "rootfs/app/main.py").read_text(encoding="utf-8")
    assert "def report_input_readiness(" in source
    assert "historical: bool = False" in source
    assert '"core_metrics": core' in source
    assert "report_input_readiness(month_key, options, historical=historical_mode)" in source
...
def test_v780_old_product_test_is_not_presented_as_current_error():
    ...
    assert "Laatste test hoort bij productiekern" in source
...
def test_v940_core_certificate_model_present():
    source = MAIN.read_text(encoding="utf-8")
    assert 'PRODUCTION_CORE_REVISION = "9.4-core3"' in source
...
def test_v101_infrastructure_foundation_present():
    source = MAIN.read_text(encoding="utf-8")
    assert 'NAS_SHARE_ROOT, NAS_LAYOUT_ROOT = resolve_nas_roots()' in source
...
def test_v1058_uses_live_epex_project_source_and_real_format():
    source = (ROOT / "slimmemeterportal_import/rootfs/app/main.py").read_text(encoding="utf-8")
    assert 'runtime_data / "05_Maanddata" / "EPEX"' in source
    assert 'runtime_data / "EPEX"' in source
...
def test_v10510_epex_uses_actual_share_root_first():
    source = (ROOT / "slimmemeterportal_import/rootfs/app/main.py").read_text(encoding="utf-8")
    expected = 'runtime_share / "05_Maanddata" / "EPEX"'
    assert expected in source
    assert source.index(expected) < source.index('runtime_data / "05_Maanddata" / "EPEX"')
    assert 'candidate / "EPEX_index.csv"' in source
    assert '"resolved_path": str(history_root)' in source
...
def test_v10511_keeps_epex_10510_fix():
    source = (ROOT / "slimmemeterportal_import/rootfs/app/main.py").read_text(encoding="utf-8")
    assert 'runtime_share / "05_Maanddata" / "EPEX"' in source
    assert '"resolved_path": str(history_root)' in source
...
def test_v10512_epex_autodetects_ha_storage_roots():
    ...
    assert '"source_found": history_root is not None' in source
...
def test_v10519_supplier_context_present():
    ...
    assert '"contract_start": "2026-09-03"' in source
    assert '"contract_end": "2027-09-03"' in source
    assert '"gas_pricing": "price_ceiling"' in source
    assert '"gas_price_ceiling_eur_per_m3": 0.8558' in source
...
def test_v1061_contract_all_in_validation_layer():
    ...
    assert '"schema": "nextenergy_contract_validation_v2"' in source
    assert '"policy": "official_contract_values_only_no_assumptions_invoice_actuals_separate"' in source
...
def test_v2060_changelog_latest_only():
    c=(ROOT/"slimmemeterportal_import/CHANGELOG.md").read_text(encoding="utf-8")
    assert f"## {CURRENT_RELEASE}" in c and c.count("\n## ") == 1
...
def test_v2440_version_and_changelog_consistency():
    ...
    assert f'version: "{CURRENT_RELEASE}"' in config
    assert f'APP_VERSION = "{CURRENT_RELEASE}"' in main
    assert f"## {CURRENT_RELEASE}" in changelog and changelog.count("\n## ") == 1
...
def test_v3203_home_assistant_github_publisher_uses_dedicated_worktree():
    ...
    assert 'GitHub-push uitgevoerd maar remote/targetverificatie wijkt af' in source
...
def test_v32023_ha_app_owns_processed_retention():
    ...
    for required in [
        ...
        'HA-app processed-retentie v%s: OK',
        'HA-app processed-retentie v%s: FOUT',
    ]:
        assert required in source
