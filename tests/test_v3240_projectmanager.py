...
def test_v3240_identity_and_embedded_projectmanager():
    assert (ROOT/"VERSIE.txt").read_text().strip()=="32.4.1"
    assert 'version: "32.4.1"' in (ROOT/"slimmemeterportal_import/config.yaml").read_text()
    app=ROOT/"slimmemeterportal_import/rootfs/app"
    mode=(app/"mode_entrypoint.py").read_text()
    assert 'TARGET_RELEASE_VERSION = "32.4.1"' in mode
