...
def test_32426_release_identity_is_synchronized():
    ...
    assert 'APP_VERSION = "32.4.47"' in (APP / "main.py").read_text(encoding="utf-8")
    ...
    assert (APP / "projectmanager_v2/VERSION.txt").read_text(encoding="utf-8").strip() == "2.0.0-rc34"
...
def test_32426_changelog_describes_indexed_clearup_dependency_audit():
    ...
    assert root_log.startswith("## 32.4.47")
