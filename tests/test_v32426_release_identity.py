...
def test_32426_release_identity_is_synchronized():
    assert (ROOT / "VERSIE.txt").read_text(encoding="utf-8").strip() == "32.4.27"
    assert 'version: "32.4.27"' in (ROOT / "slimmemeterportal_import/config.yaml").read_text(encoding="utf-8")
    assert 'APP_VERSION = "32.4.27"' in (APP / "main.py").read_text(encoding="utf-8")
    assert 'TARGET_RELEASE_VERSION = "32.4.27"' in (APP / "mode_entrypoint.py").read_text(encoding="utf-8")
    assert contract.CURRENT_RELEASE == "32.4.27"
...
def test_32426_changelog_describes_indexed_clearup_dependency_audit():
    ...
    assert root_log.startswith("## 32.4.27")
