...
def test_v3244_release_and_manager_identity():
    ...
    assert 'APP_VERSION = "32.4.9"' in (APP / 'main.py').read_text()
    assert 'TARGET_RELEASE_VERSION = "32.4.9"' in (APP / 'mode_entrypoint.py').read_text()
    assert (PM / 'VERSION.txt').read_text().strip() == '2.0.0-rc6'
    assert (ROOT / 'CHANGELOG.md').read_text().startswith('## 32.4.9')
