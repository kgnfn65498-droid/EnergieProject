...
def test_v32328_release_identity_is_consistent():
    ...
    assert 'APP_VERSION = "32.4.2"' in MAIN.read_text(encoding="utf-8")
    assert 'TARGET_RELEASE_VERSION = "32.4.2"' in (APP / "mode_entrypoint.py").read_text(encoding="utf-8")
    assert (ROOT / "CHANGELOG.md").read_text(encoding="utf-8").startswith("## 32.4.2")
