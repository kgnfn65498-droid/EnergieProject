...
def test_v32035_release_identity_is_synchronized():
...
def test_v32035_changelog_mentions_smp_report_fallback():
    changelog = CHANGELOG.read_text(encoding="utf-8")
    assert changelog.startswith("## v32.0.35 — Pagina 2 onbekende terugleververgoeding")
    assert "feed_in_compensation" in changelog
    assert "€0" in changelog
    assert "finalize_month" in changelog


def test_v32035_addon_changelog_is_current_release_only():
    changelog = ADDON_CHANGELOG.read_text(encoding="utf-8")
    assert "## 32.0.35 - Pagina 2 onbekende terugleververgoeding" in changelog
