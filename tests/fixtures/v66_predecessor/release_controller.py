...
class Phase(str,Enum):
    DETECTED='DETECTED';VERIFIED='VERIFIED';PUBLISHING='PUBLISHING';INSTALLING='INSTALLING';INSTALLED='INSTALLED'
...
class ReleaseState:
    ...
    phase:str;status:str;step:int;total:int=9;blocker:str='';required_action:str='';wait_reason:str=''
...
class Adapter(Protocol):
    def pre_target_publication(self,s:ReleaseState)->Outcome:...
...
class ReleaseController:
    ...
    def cycle(self,s,adapter):
        ...
        # PUBLISHING/INSTALLING/RUNTIME_ALIGNING durable authorization fences across crashes.
        if s.phase==Phase.VERIFIED.value:
            self._phase(s,Phase.PUBLISHING);return s
        if s.phase==Phase.PUBLISHING.value:
            return self._outcome(s,adapter.pre_target_publication(s),Phase.INSTALLING,adapter,rollback_allowed=False)
