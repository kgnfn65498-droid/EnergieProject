...
class ManagerService:
    ...
    def _sync_managed_documents(self, status: dict):
        ...
        master_content = '\n'.join(truth_header + [
            ...
            '## Ontwikkelproceslessen 32.4.36',
            '- CR-retentie is max-1 per type en nieuwe sets gebruiken datum/tijd + runtimeversie + CR + type; TLS-certificaatroutes zijn geen actieve NAS-CR-oplossing.',
            '- CLEARUP hergebruikt nooit een stale plan; verse dependency-audit en hard-move/no-delete blijven verplicht.',
            '## Ontwikkelproceslessen 32.4.37',
            '- Een proces-reexec wijzigt geen Docker-mounts/capabilities; watcher-containerconfig heeft een expliciet contract en meldt RECREATE_REQUIRED in plaats van schijn-GREEN.',
            '- Native MCP broncode op schijf is niet gelijk aan geladen runtime; source/runtime fingerprint en Peter-goedgekeurde fixed-target reload sluiten dat gat.',
            '- CR-retentie verplaatst oude geldige sets eerst herstelbaar naar quarantaine en wist niet direct.',
            '## Ontwikkelproceslessen 32.4.38',
            '- Een kort signaal zoals `vXX draait` start automatisch de build-specifieke live-acceptancecontrole; PM wacht niet op een tweede hint.',
            '- CLEARUP voor 32.4.38+ start pas na actuele canonieke EnergieProject- én NAS-CR en hergebruikt alleen een manifest met dezelfde prerequisite fingerprint.',
