...
VERSION="$(tr -d '\r\n ' < "$ROOT/App/VERSIE.txt")"
[ "$CONFIRMATION" = "RECREATE WATCHER $VERSION" ] || { echo "FOUT: bevestiging moet exact zijn: RECREATE WATCHER $VERSION" >&2; exit 3; }
...
grep -Fq "\"release_version\": \"$VERSION\"" "$REQUEST" || { echo "FOUT: request release" >&2; exit 8; }
...
grep -Fq "\"confirmation_required\": \"RECREATE WATCHER $VERSION\"" "$REQUEST" || { echo "FOUT: request confirmation" >&2; exit 11; }
sh "$BOOTSTRAP"
N=0
while [ "$N" -lt 20 ]; do
  if [ -f "$MARKER" ] && grep -Fq '"ready": true' "$MARKER" && grep -Fq '"contract_version": 3' "$MARKER"; then
    rm -f "$REQUEST"
    echo "WATCHER_RECREATE_GREEN release=$VERSION"
    exit 0
  fi
  sleep 1
  N=$((N+1))
