...
def authorize_release_native_reconcile(*,request_path:Path,controller_state_path:Path,version_path:Path,runtime_guard_path:Path)->tuple[dict,dict]:
    ...
    if not validate_release_scoped_request(request,state,live_version=live_version,expected_fingerprint=expected):
        raise RuntimeError('release-scoped native MCP reconcile authorization rejected')
    return request,state

def authorize_release_native_request(*,request_path:Path,controller_state_path:Path,version_path:Path,runtime_guard_path:Path)->tuple[dict,dict]:
