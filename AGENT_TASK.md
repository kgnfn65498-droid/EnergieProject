...
## 2026-09-21 — Spock executor proof GREEN; exact Work source export is next
Spock has now proven a silent local executor path without GitHub Actions, PRs, NAS/HA, production or external notifications.

### Executor proof
Basis used only for executor proof:
- candidate source commit: `33bea32534c5114aefe822afe252a6555bc55e58`
- NOT a release-ready 60; identity remains 32.4.59 on that proof source.

Clean isolated venv:
- repository `sitecustomize.py` wins over Chat host hooks;
- repository offline guard active in parent and child Python;
- no private NAS/HA access used.

Source suite after two compatibility fixes below:
- collected: 1981
- passed: 1979
- skipped: 2
- failed: 0
- executed in bounded batches only because one monolithic invocation exceeds the host execution timeout; no test was omitted.

Exact compatibility fixes discovered by the full suite:
1. `tests/test_v32457_required_acceptance.py`
   - historical `GreenAdapter` must implement `pre_target_publication()` returning GREEN `publish`;
   - expected call sequence becomes `publish, install, runtime, verify, accept, delivery`.
   - This preserves/extends the assertion for the new durable PUBLISHING phase; it does not weaken the test.
2. `tests/test_v32455_watcher_practical_soak.py::test_watcher_restart_before_claim_never_double_claims`
   - set `ENERGIE_ZIP_STABLE_POLLS=20` in that test before the first watcher start.
   - Reason: make the intended pre-restart no-claim window explicit instead of depending on incidental executor startup slowness. Invariant remains: zero claim before restart, exactly one claim after restart.

Canonical build capability:
- release_artifact_builder: GREEN
- ZIP CRC: GREEN
- exact fresh-extract: GREEN
- manifest/SHA256SUMS required files: GREEN
- proof artifact SHA256: `1cdd4e9a6fa89c9183661ffdeaec956001c31490dcc34a1380c147f71df4b70b`
- proof artifact is NOT a release candidate and must never enter Incoming.

Fresh-extract full suite with the same two compatibility fixes:
- passed: 1979
- skipped: 2
- failed: 0
- all 1981 collected tests accounted for.

### EXECUTOR_READINESS_MATRIX update
| Fase | Status | Bewijs |
|---|---|---|
| Offline parent/child guard | GREEN | clean isolated venv, repo sitecustomize active |
| Full pytest execution capability | GREEN | all 1981 source tests executed; 1979 pass + 2 skip after two explicit compatibility fixes |
| Canonical build | GREEN | release_artifact_builder + CRC |
| Exact fresh-extract | GREEN | required payload/metadata present |
| Fresh-extract full pytest capability | GREEN | 1979 pass + 2 skip |
| GitHub Actions | FORBIDDEN | not needed; user explicitly forbids unapproved runs/notifications |
| Exact current Work 60 source availability to Spock | BLOCKED | checkpoint 29da2de is only inside Work runtime and not present as a GitHub branch |

### Next bounded Work action: SOURCE EXPORT ONLY
Work must NOT rerun the full suite in its blocked runtime.
Work must publish the exact current branch/checkpoint source to GitHub so Spock can run the proven executor against the actual 32.4.60 source.

Required exact action:
1. Resume checkpoint `29da2de404299cc0a4a49224ba431fec5b0feb4c`.
2. Verify current branch is `work/replacement-32.4.60-combined` and working tree contains the exact checkpoint state.
3. No code/test edits during export.
4. Push/publish that exact branch to GitHub as `work/replacement-32.4.60-combined`.
5. NO pull request, NO GitHub Actions/workflow, NO tag/release, NO merge, NO force push, NO main update, NO NAS/HA/production action.
6. Read back the remote branch head SHA and compare it to the exact local exported state.
7. Write `SOURCE_EXPORTED_FOR_SPOCK_EXECUTOR` plus remote branch head SHA to AGENT_RESULT/WORK_LEDGER if possible without changing the exported branch content.
8. Stop only after exact remote readback is GREEN or a genuine GitHub publication blocker is proven.

Risk/side-effect note:
- A plain branch push may appear in repository activity, but it must not intentionally create PR/workflow notifications.
- Because main currently has no .github/workflows and this instruction forbids adding/running one, no CI mail should be generated by this export.
